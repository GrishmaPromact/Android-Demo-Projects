 ...
 */
public class PostResponseFromServerTask extends AsyncTask<String,String,String> {
    private Context mContext;
    private DownloadJsonString<ArrayList<Info>> callback;
    public PostResponseFromServerTask( Context context, DownloadJsonString<ArrayList<Info>> cb) {
        mContext = context;
        this.callback = cb;
    }
    @Override
    protected String doInBackground(String... strings)
    {
        String response="";
        try {
             response=new ApiClient().sendPost();
            return response;
        } catch (Exception e) {
            e.printStackTrace();
            return response;
